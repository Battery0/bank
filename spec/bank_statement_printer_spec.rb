...
describe BankStatementPrinter do
  ...
  let(:transaction_dbl) { double(:transaction_double, { record: { date: date, credit: "£500.00", debit: "", balance: "£500.00" } }) }
  let(:formatted_statement) { "#{date} || £500.00 ||  || £500.00" }

  describe '#print_statement' do
    it 'should output formatted bank statement' do
      expect { print_bank_statement.print_statement([transaction_dbl]) }.to output("#{statement_header}\n#{formatted_statement}\n").to_stdout
    end
  end
